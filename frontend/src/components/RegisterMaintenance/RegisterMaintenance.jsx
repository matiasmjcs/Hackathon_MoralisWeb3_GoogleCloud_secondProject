...
const RegisterMaintenance = ({close}) => {
  ...
  const registerMaintenance = async () => {
    try {
      ...
      if (ethereum) {
        ...
        const tx = await contract.registerMaintenance(
          input.Patent,
          input.Date,
          input.Direction,
          input.Description
  ...
  const handleSubmit = (e) => {
    ...
    if (!input.Direction.trim()) {
      e.target.Direction.focus()
      return Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'do not leave the Direction field empty',
      })
    }
    if (!input.Description.trim()) {
      ...
      return Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'do not leave the year Description empty',
      })
    }
